...
import ir.sahrapanel.app.core.data.data_source.local.db.entity.UserTokenEntity
...
interface UserTokenDao {
    ...
    suspend fun insertOrUpdateToken(userToken: ir.sahrapanel.app.core.data.data_source.local.db.entity.UserTokenEntity)

    // 2. Get the current user token as a Flow for real-time UI updates
    @Query("SELECT * FROM user_tokens LIMIT 1")
    fun observeCurrentUserToken(): Flow<ir.sahrapanel.app.core.data.data_source.local.db.entity.UserTokenEntity?>

    // 3. One-shot fetch for the current user token (useful in Interceptors/Repositories)
    @Query("SELECT * FROM user_tokens LIMIT 1")
    suspend fun getCurrentUserToken(): ir.sahrapanel.app.core.data.data_source.local.db.entity.UserTokenEntity?
